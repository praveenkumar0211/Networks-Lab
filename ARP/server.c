/* Program to implemant Address resolution protocol ( arp ) in C Language */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int k, i, len, count;
fd_set newfds;
arp packet, recv_packet;packet = createARPPacket(REQ);
...
while(1){
FD_ZERO(&newfds);
FD_SET(sockfd, &newfds);
//Clears socket set.
//Add sockfd to socket set.
max = sockfd;
for(i = 0; i < 10; i++){
fd = client_sockets[i];
if(fd > 0){
...
//Store the max valued FD.//Wait indefinitely till any client pings.
